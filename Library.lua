...
function LunarShell:Init(options)
    ...
    closeBtn.MouseButton1Click:Connect(function()
        -- Criar overlay de confirmação (centralizado)
    ...
    function LunarShell:Category(name, icon)
        ...
        if not currentCategory then
            -- Simular clique com um pequeno delay
            task.spawn(function()
                task.wait(0.1)
                catBtn:Click()
            end)
        ...
        function categoryAPI:AddToggle(name, defaultState, callback)
            ...
            -- Usar TextButton para o toggle (corrige o erro MouseButton1Click)
        ...
        function categoryAPI:AddSlider(name, min, max, default, callback)
            ...
            local knob = Instance.new("TextButton")  -- Botão para arrastar
